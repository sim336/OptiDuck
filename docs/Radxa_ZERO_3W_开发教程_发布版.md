# Radxa ZERO 3W 机器人开发实战：模型训练、上板部署与 Rust 开发

**摘要**：本文承接《[Radxa_ZERO_3W_部署调试教程_发布版.md](./Radxa_ZERO_3W_部署调试教程_发布版.md)》，讲的是"把软件装上之后"的事 —— 自己训练一个策略模型、导出并部署到 Radxa ZERO 3W 上、调试舵机与各类传感器、改一行 Rust 代码并在一分钟内推到板子生效。全文覆盖三条链路：**模型链路**（训练 → 导出 ONNX → CPU 彩排 → 发布/占槽 → 上板）、**硬件链路**（舵机总线、IMU、ToF、摄像头的体检与排障）、**代码链路**（`dev-push` 的交叉编译、签名、健康门与回滚）。文中所有命令、寄存器值、版本号、报错原文与判定阈值均来自实机记录，可直接照做。**前置条件**：已按部署篇把板子配好（能 SSH、`robotctl` 已装、7 个守护进程在跑），并具备 Linux 命令行、SSH、PowerShell 的基本使用能力，对强化学习（PPO）与 Rust 有初步了解。文末附常见问题速查表、参考资料索引、参考来源、待核实事项与更新日志。
...
| ---- | ---- |
| **【无需硬件】** | 不接舵机 / 传感器也能做完 |
| **【需硬件】** | 必须硬件就绪（舵机、摄像头、ToF HAT 等）才可执行 |
| **【原理】** | 解释性内容，赶时间可跳过 |
| **⚠️** | 已知的坑、或容易失败的地方 |

> **关于路径**：下文出现的 `~/microduck`（上板软件栈仓库）、`~/microduck_rl`（强化学习训练仓库）以及 `~/OpenMicroDuck`（硬件资料仓库）都是**示例路径**，请替换为你自己的本地工作目录；出现 `C:\Users\<你的用户名>` 的地方同理。出现 `<板子IP>` 的地方，替换为板子的局域网地址。本教程使用的参考板为 Radxa ZERO 3W（RK3566 / 2GB LPDDR4 / 无 eMMC），实测数据见各节标注 —— **不同批次的板子可能不同，请以自己板子的实测为准**。

## 本文目录

- [0. 开篇：路线图与三个仓库](#0-开篇路线图与三个仓库)
  - [0.1 三条链路与心智模型](#01-三条链路与心智模型)
  - [0.2 三个仓库、一个外部分发站](#02-三个仓库一个外部分发站)
- [1. 先建立全局观：这个系统是怎么长在一起的](#1-先建立全局观这个系统是怎么长在一起的)
  - [1.1 一张图认识 7 个守护进程](#11-一张图认识-7-个守护进程)
  - [1.2 两个必须分清的词：`slot` 与 `skill`、`policy` 与 `detector`](#12-两个必须分清的词slot-与-skillpolicy-与-detector)
  - [1.3 三条更新通道：什么走哪条，为什么](#13-三条更新通道什么走哪条为什么)
  - [1.4 三板斧（先记住，后面反复用）](#14-三板斧先记住后面反复用)
- [2. 模型部署：从训练到上板（全链路）](#2-模型部署从训练到上板全链路)
  - [2.1 训练一个走路策略（最短路径）](#21-训练一个走路策略最短路径)
  - [2.2 导出 ONNX（归一化会被烤进模型）](#22-导出-onnx归一化会被烤进模型)
  - [2.3 CPU 彩排：上真机前的最后一次检查](#23-cpu-彩排上真机前的最后一次检查)
  - [2.4 发布到 HF Hub：一个模型变成一个"仓库"](#24-发布到-hf-hub一个模型变成一个仓库)
  - [2.5 策略上板：五条命令全场景](#25-策略上板五条命令全场景)
  - [2.6 检测器模型：另一条链（YOLO → RKNN）](#26-检测器模型另一条链yolo--rknn)
  - [2.7 NPU 推理链路：模型在板子上到底怎么跑](#27-npu-推理链路模型在板子上到底怎么跑)
  - [2.8 两条模型链路对照表](#28-两条模型链路对照表)
- [3. 硬件调试：从体检到动起来](#3-硬件调试从体检到动起来)
  - [3.0 硬件地图：这条鸭子身上有什么](#30-硬件地图这条鸭子身上有什么)
  - [3.0.1 舵机选型：HD1910 与官方 XL330](#301-舵机选型hd1910-与官方-xl330)
  - [3.0.2 采购清单：硬件到手前先对一遍（OpenMicroDuck BOM）](#302-采购清单硬件到手前先对一遍openmicroduck-bom)
  - [3.1 硬件体检【无需硬件】：先养成这个习惯](#31-硬件体检无需硬件先养成这个习惯)
  - [3.2 舵机总线调试【需硬件】：Dynamixel 链](#32-舵机总线调试需硬件dynamixel-链)
  - [3.3 IMU【需硬件】：姿势的裁判](#33-imu需硬件姿势的裁判)
  - [3.4 ToF【需硬件】：HAT 上的 8×8 深度矩阵](#34-tof需硬件hat-上的-88-深度矩阵)
  - [3.5 摄像头【需硬件】：看世界的眼睛](#35-摄像头需硬件看世界的眼睛)
  - [3.6 音频【需硬件】：一句话带过](#36-音频需硬件一句话带过)
  - [3.7 常见硬件故障速查表](#37-常见硬件故障速查表)
- [4. 软件开发：改代码，推到板子上](#4-软件开发改代码推到板子上)
  - [4.1 代码结构：一个 workspace，七个 daemon](#41-代码结构一个-workspace七个-daemon)
  - [4.2 一次性准备（只在第一台板子做一次）](#42-一次性准备只在第一台板子做一次)
  - [4.3 dev-push 闭环：改一行 → 板子上看到它](#43-dev-push-闭环改一行--板子上看到它)
  - [4.4 第一个真实改动：加一条"被摸"日志](#44-第一个真实改动加一条被摸日志)
  - [4.5 示例项目：spaces/ 里有什么](#45-示例项目spaces-里有什么)
  - [4.6 调试三板斧（按顺序用）](#46-调试三板斧按顺序用)
  - [4.7 dev-push 常见错误速查](#47-dev-push-常见错误速查)
- [5. 端到端演练：给鸭子加一个"被摸会回应"的行为](#5-端到端演练给鸭子加一个被摸会回应的行为)
- [6. 常见问题速查](#6-常见问题速查)
- [7. 参考资料索引（按主题）](#7-参考资料索引按主题)
- [参考来源](#参考来源)
- [待核实事项](#待核实事项)
- [更新日志](#更新日志)
...
这一节先给出全局图景：三条链路各自解决什么问题，以及手上有哪几个代码仓库。**建议在动手前先通读一遍**，后面每一节都能对回这张图。
...
flowchart LR
  subgraph M["模型链路 §2"]
    M1["训练 PPO<br/>microduck_rl"] --> M2["导出 ONNX<br/>scripts/export.py"] --> M3["转 RKNN<br/>检测器<br/>to_rknn.py"] --> M4["发布到 HF Hub<br/>policies /<br/>duck-detector"]
  end
  subgraph C["代码链路 §4"]
    C1["改代码"] --> C2["交叉编译"] --> C3["dev-push.sh<br/>打包 + 签名 + apply + 健康门"]
  end
  subgraph H["硬件链路 §3"]
    H1["robotctl<br/>health / monitor"]
    H2["舵机 · IMU · ToF · 摄像头"]
  end
  M4 -->|updaterd| N1["/opt/robot/policies/current<br/>/opt/robot/detector/current"]
  C3 --> N2["/opt/robot/daemon/current<br/>release 版本"]
  N1 --> N3["robotd<br/>50 Hz 控制循环"]
  N2 --> N3
  N3 --> H2
  H1 -.->|观测| N3
```

> **图 0**：三条链路各自的起点与终点 —— 模型与代码都是"往上送"（送到板子的 `current` 符号链接），硬件则是被 `robotd` 驱动、被 `robotctl` 观测的对象。三条链路的关系在这里一眼可见，后面每节都能对回这张图。

一句话版本：

- **模型 = 被动数据**（`.onnx` / `.rknn` + `manifest.json`），走 **HF Hub → updaterd** 通道，更新**不重启 daemon 之外的任何东西**，随时可以回退；
- **代码 = 主动逻辑**（Rust 二进制 + 板级脚本），走 **dev-push / release** 通道，每次替换都要过**签名校验 + 健康门 + 自动回滚**；
- **硬件 = 被调试的对象**，全部以 `robotctl health` / `monitor` 为观测入口。

> 💡 **部署篇讲"装"，本篇讲"造"。** 部署篇里出现过的 `policies/current → seed-v5`、`detector/current → seed-duck-v1` 这两个符号链接，就是模型链路的终点 —— 读完 §2 你会知道它们是怎么被填上的、以及如何填上自己的模型。
...
| 仓库 | 示例本地路径 | 干什么 |
| ---- | ---- | ---- |
| `microduck` | `~/microduck` | **上板软件栈**：7 个 Rust daemon、`robotctl`/`duckctl`、板级脚本。开发主要在这里 |
| `microduck_rl` | `~/microduck_rl` | **强化学习训练**：MuJoCo + PPO 训走路/站立/翻跟头，导出 ONNX |
| `duck_detector`（外部） | 在 GitHub / HF 上 | **检测器训练**：YOLO 模型训练 + ONNX→RKNN 转换（不在本地仓库里） |

还有一个"仓库"不是代码：**Hugging Face Hub**。策略（`microduck-*`）、检测器（`microduck-duck-detector`）都以"文件 + 一个 `manifest.json`"的形式发布在 HF 上，板子通过 `updaterd` 从 HF 拉取。**模型和策略的更新不经过 daemon 发版** —— 这是全篇最重要的概念之一。

对新手而言，先分清这份教程的阅读路线：

| 想做什么 | 读哪节 |
| ---- | ---- |
| 先搞清楚"鸭子系统到底怎么长在一起的" | §1 |
| 训练 / 部署一个自己的策略或检测模型 | §2（全链路） |
| 调舵机、IMU、ToF、摄像头 | §3 |
| 改 Rust 代码并推到板子上跑 | §4 |
| 把上面串起来做一个完整小功能 | §5（端到端演练） |

> 训练链路用到 `uv`（一个包与命令管理器，可为每条命令单独创建隔离环境，速度快、兼容性好）；代码链路用到 Rust 工具链与 `cargo`。这些工具的具体安装方法在各节分别给出。

## 1. 先建立全局观：这个系统是怎么长在一起的

### 1.1 一张图认识 7 个守护进程

部署篇只讲"如何装上"，没有说明它们各自是什么。这里补充（来自 `docs/design/architecture.md`）：

```mermaid
flowchart LR
  subgraph CORE["JSON-RPC 2.0 · 一行的 NDJSON · unix socket"]
    R1["robotd（50 Hz 控制循环，唯一能碰电机的人）"]
    R2["configd（Wi-Fi/名字/配对）"]
    R3["tofd（ToF 8×8 深度矩阵，只发不收）"]
  end

  E1["手柄"] -->|"BLE"| P1["padd"]
  E2["手机"] -->|"BLE"| P2["btd"]
  E3["SSH（robotctl）"] -->|"ssh"| P3["robotctl"]
  E4["远端"] -->|"WebRTC"| P4["mediad"]
  E5["GitHub release"] -->|"https"| P5["updaterd"]
  ...
  CORE -->|"一条 Dynamixel UART（/dev/ttyS2）"| M1["15 个舵机 + IMU（id 200，本教程参考板使用的 HD1910）"]
```

> **图 1**：7 个守护进程与外部入口的关系——外部入口经 unix socket 接入板内的 `robotd` / `configd` / `tofd`，再由 `robotd` 通过一条 Dynamixel UART 驱动舵机与 IMU。

**三个必须记住的硬规则**（后面调试全靠它们）：

1. **`robotd` 是唯一能命令电机的人。** 其他进程只能发"意图"（走多快、看哪里、站起来），安全性由 `robotd` 裁决。所以舵机总线出问题，先怀疑 `robotd`。
2. **`configd` / `updaterd` / `btd` 不依赖 `robotd`。** 机器人控制循环挂掉时，仍可改 Wi-Fi、更新、回滚——这是"永不砖"的根基。
...
> 目标：本节结束后，可以把一个**自己训出来的模型**（或仓库里已有的模型）放到鸭子上跑起来，
> 并知道每一步卡住时去哪查。全链路五段：

```mermaid
flowchart LR
  A1["① 训练（microduck_rl）：uv run train"] --> A2["② 导出 ONNX：export.py"]
  A2 --> A3["③ 彩排（CPU）：infer_policy / policy-rehearsal"]
  A3 -.->|"可选分叉：要叫名字 / 绑手柄 / 分享"| A4["④ 发布到 HF Hub：uv run publish"]
  A3 -->|"只在自己板子上用（local 来源）"| A5["⑤ 上板占槽 / 发布：robotctl policy / duck-detector"]
  A4 --> A5
  A5 --> A6["板子上生效"]
```

> **图 2**：从训练到板子上生效的完整链路，其中"④ 发布到 HF Hub"是可选的虚线分叉。

**④ 的取舍**是：自己板子上用 → 跳过 ④，ONNX 拷到板子直接占槽位（local 来源）；要叫名字/绑手柄/分享 → 先 ④（`uv run publish`），再装成技能。

策略和检测器是**两条平行的链**（训练仓不同、转换不同、上板命令不同），分开讲，最后汇总一张对照表。（uv 是一个包及命令管理器，可以为每个命令单独创建环境运行，速度快、兼容性好）
...
**第一目标是跑通冒烟测试**，不是训出好策略——冒烟测试 1–2 分钟，能拦下约 95% 的配置错误
...
`microduck_rl/vendor/bam/bam/params/hd1910/*.json`（本教程参考板使用的型号）/ `hls2909`（另一款）。
...
> 💡 **当前板子实况：** 板子上 `policies/current → seed-v5` 是官方策略集。训练的第一步
...
例如需要导出以下目录中的模型：`~/microduck_rl/logs/rsl_rl/velocity/2026-09-15_20-44-58_prebom_resume`
...
uv run scripts/export.py Mjlab-Velocity-Flat-MicroDuck `
  --checkpoint-file "~/microduck_rl/logs/rsl_rl/velocity/2026-09-15_20-44-58_prebom_resume/model_7750.pt" `
...
下面分开讲。多数人会栽在②的一个认知上，先看②。

```mermaid
flowchart TD
  S0["CPU 彩排：上真机前的最后一次检查"] --> S1["① 训练仓彩排 — 看动作对不对（开发机 MuJoCo 仿真，infer_policy.py）"]
  S1 --> D1{"动作合不合理？能不能走 / 站 / 起身？"}
  D1 -->|"否"| F1["回到训练 / 导出（§2.1、§2.2）"]
  D1 -->|"是"| S2["② 上板仓彩排 — 测延迟（板子上跑 policy-rehearsal）"]
  S2 --> D2{"p95 / p99 / max 稳在 20 ms 内，over_20_ms 为 0？"}
  D2 -->|"否"| F2["不能上板：延迟超标，先排查"]
  D2 -->|"是"| F3["通过彩排，进入 §2.4 发布"]
```

> **图 3**：两条彩排子路径及其判定——先看动作（行为对不对），再测延迟（性能够不够），顺序不能换。
...
它会弹出一个 3D 查看器，可用方向键驱动它，直观确认策略行为正常。
...
开发机的 CPU 比板子快得多，拿开发机的 p50 去判断"能不能跑 50Hz"，
...
scp ~/microduck_rl/standup_hd1910.onnx radxa@192.168.31.30:/home/radxa/
...
> 注意：本地 `~/microduck` 这个目录**也只是官方仓库的一份 clone**（remote 指向
> `pollen-robotics/microduck`，本地无私有提交）——GitHub 上并不存在独立的第三方版上板仓。
...
> 💡 **先分流：发布不是必经之路**。如果模型只在本地板子上使用，可以跳过本节，
> 直接走 §2.5 的 `sudo robotctl policy load <槽位> /home/radxa/xxx.onnx`（本地文件占槽，
> 持久生效）。走发布的意义：叫名字触发（`robot do`）、绑手柄按钮、分享给他人、
> 有 manifest 校验和版本管理。两条路见 §2.5 末尾的对比。

**发布前置：HF 登录（一次性的）。** `uv run publish` 走 `huggingface_hub` 上传，
token 从两个地方找，**没有 token 就无法发布**（报 `no cached HF token. Run hf auth login.`）：

> ⚠️ **注意**：本节的登录、发布命令都在开发机上执行（发布模型的是开发机）。板子**不需要**
...
4. **Presets**（预设按钮）：点 **⬆ Write** —— 这正是发布需要的权限集（对账号名下仓库的
...
# 打印出 HF 用户名 = 登录成功；报错 = token 没配上
```

**token 权限要求**：`publish` 要在账号名下发仓库（`create_repo`）并上传文件
（`upload_folder`），必须用 **Write 权限**的 token（创建页 Presets 点 **⬆ Write** 即可；
只读 token 上传会 401/403）。`--repo <user>/microduck-<name>` 里的 `<user>` 就是
登录账号的 HF 用户名，publish 会**自动从 token 推断**——所以先登录、`<user>` 用自己账号的用户名。

> 💡 如果之后用 `uv run train ... --hf-jobs` 在 HF 上跑训练，那是另一个
...
**manifest 不需要手写**——训练仓的 `publish` 命令根据参数**自动生成**它，并做
上传前体检（61→14 维度校验 + NaN 冒烟测试），不合格不会传（`microduck_rl/AGENTS.md`）。
权重来源有两种（**下述两种来源仅作解释，已在 §2.2 导出 ONNX 时执行过**）：
...
- **`--slot` 只是"显示用"**（`Display-only; drives the install hint`）：它只决定上面打印哪条
  安装提示，**不会**自动锁死槽位。真锁槽位的是板子上那条 `policy load`。
...
| `--repo <用户名>/microduck-standup` | 发到 HF 账号下；**技能名 = `microduck-` 后面的部分**（`standup`）   |
...
**另一种装法：不发技能，改走槽位（同一份 ONNX，换个 `kind`）**。若希望起身模型像
...
> 只是**展示字段**（官方例子写 `xl330`，本项目参考板写 `hd1910`），不校验但写对才能溯源。
> 官方 `policy-manifest.md` 里的例子、以及 `robotctl policy add` 的校验规则都基于 `robot.model`。

**两个最关键的概念字段**（`kind` 和 `command.encoding`，板子靠它们决定怎么用这个模型）：
...
> ⚠️ **注意**：**`<源>` 的判定规则（踩过坑）**：`robotctl` 先看**这个路径在板子上存在吗**——
...
> ⚠️ **注意**：这条不是"临时试试"——`policy load` 会**写入 `/etc/robot/robotd.toml` 持久生效**，
> 重启后仍指向该文件（实测确认）。所以文件**必须一直留在板子上**——文件丢了，槽位会在
> 启动时回落到官方策略并在 `robotctl health` 里报出来。

**报错 `the robot accepted the change but had not made it after 20s` 怎么读**（参考板实测）

**这不是失败**，是"**命令层成功、应用层尚未完成**"：`robotctl` 把请求交给 `robotd`，
`robotd` 收下了，但 20 秒内没做完切换，于是先这么返回，并提示自行去看结果。要分两半看：
...
- **`walk` / `stand` 是 perpetual gait 槽**（一直跑）→ 自训步态/站姿模型直接适配。
  ...
  `sudo robotctl policy load stand /home/radxa/standup_hd1910.onnx`
  （参考板实测：配置能写入 `robotd.toml`；当时仅因电机总线超时未完成 live 切换）
...
**要叫名字触发/绑手柄/分享给他人 → 走发布 + 技能**。区别只在触发机制，模型本身没有
...
以及非 `constant` 编码——任何一项不符直接拒绝，**不会动现有策略**。
...
```text
/opt/robot/detector/current      → releases/seed-duck-v1/   # 检测器（参考板当前安装的是这个）
...
```mermaid
flowchart TD
    A1["上游模型：yolo11n 320×320（duck_detector 仓训练）"]
    A2["scripts/to_rknn.py：INT8 量化"]
    A3["duck_detector.rknn（NPU 主路径）"]
    A4["duck_detector.onnx（CPU 兜底）"]
    A5["发布：HF Hub 同名仓库两个文件"]
    A6["上板：/opt/robot/detector/current"]
    A7["板端选择：优先 .rknn，NPU 不可用时回退 CPU 的 .onnx（deploy/robotd.toml L348-358）"]
    A8["duck-detect crate（duck-detect/src/rknn.rs）"]
    A9["librknnrt.so v2.3.2（dlopen 动态加载）"]
    A10["rknn_init → rknn_outputs_get（运行时反量化成 float）"]
    A11["解码层"]
    A12["mediad（摄像头侧）"]
    A1 --> A2
    A2 --> A3
...
> **图 4**：检测器从训练产物到板端调用的完整链路——ONNX 经 `to_rknn.py` INT8 量化成 RKNN，由 `librknnrt.so` 在 NPU 上执行，最终由 `mediad`（经 `duck-detect` crate）调用。
...
不是它的 0.5。一跑检测不到东西，先试 `--threshold 0.2`，不要急于怀疑转换出错。
...
> ⚠️ **当前状态（2026-09）**：参考板尚未接上舵机、摄像头与 ToF HAT。本节按"硬件到手后的完整流程"组织，每一步都标注 **【无需硬件】** 还是 **【需硬件】**。硬件到齐后可直接照做；不依赖硬件的部分（总线体检、日志、排障习惯）建议先练手。
>
> 核心思想只有一个：**板上的一切硬件都由 7 个 daemon 包了一层接口，始终从 `robotctl` 这一侧观察和操作，不要绕过 daemon 直接摸硬件。** 绕过了，下一层就是"为什么它不理我"的无底洞。

### 3.0 硬件地图：这条鸭子身上有什么

> 📌 **参考板实测基线（2026-09-18，SSH 上板逐项核实，非照抄官方文档）**：
> 系统是 **Radxa OS 的 KDE 桌面版镜像**（`/etc/os-release` = `Debian GNU/Linux 12 (bookworm)`；`/proc/device-tree/model` = `Radxa ZERO 3`），
> 内核 **6.1.84-10-rk2410-nocsf**，桌面相关包已装 `kde-plasma-desktop`、`sddm`、`radxa-desktop-branding`；
> 内存 2 GB（`free -m` 1978 MB）；Wi-Fi/BT 芯片 **AIC8800**（不是 Armbian——官方 `media-bringup.md` 里那些
> Armbian 专属的坑在参考板上不存在，下文凡涉及都会区分）。
> 当前已启用的 overlay（`/boot/extlinux/extlinux.conf` 的 `fdtoverlays` 行）是
> `rk3568-i2c4-m0.dtbo`、`rk3568-npu-enable.dtbo`、`rk3568-uart2-m0.dtbo`；而 `/boot/dtbo/` 里的
> `rk3568-i2c3-m0.dtbo` 是 `.disabled`。
>
> 总线上"谁挂在哪儿"的实测快照：`/dev/i2c-*` 只有 **0 / 3 / 4 / 6**——i2c-0 上是 PMIC `rk817 @0x20`
> 与 `syr827 @0x40`，i2c-3 上是 Type-C 控制器 `fusb302 @0x22`，i2c-6 是 HDMI DDC；**`/dev/i2c-4` 当前没有从机应答**
> （HAT 未接）。另有 udev 建立的稳定别名 `/dev/i2c-pihat -> i2c-3`（规则 `/etc/udev/rules.d/99-robot-i2c-pihat.rules`）。

```text
                    Radxa ZERO 3W（参考板，Radxa OS / Debian 12）
   ┌──────────────────────────────────────────────────────────────┐
   ...
   · 舵机具体型号由机器人配置决定：参考板使用的是 HD1910（官方鸭子是 XL330，同协议不同电机）
   · 机器人没有第二根电机总线，也没用 DXL 之外的协议（docs/design/robotd-design.md §1.1）
```

```mermaid
flowchart LR
    MCU["Radxa ZERO 3W 主控"]
    MCU --> UART["UART2 /dev/ttyS2（半双工换向后）"]
    UART --> SERVO["15 × Dynamixel 舵机（左腿 20–24 / 右腿 10–14 / 头颈嘴 30–34）"]
    UART --> IMU["IMU 板 id 200（imu_to_dxl，与舵机共用串口）"]
    MCU --> I2C4["i2c-4（overlay 已启用）"]
    I2C4 --> CODEC["TLV320AIC3104 codec（HAT 未接）"]
    I2C4 --> TOF["ToF VL53L5/8CX，8×8 深度矩阵（HAT 未接）"]
    MCU --> I2C3["i2c-3（基础总线，udev 别名 /dev/i2c-pihat）"]
    I2C3 --> FUSB["Type-C 控制器 fusb302 @0x22"]
    MCU --> NPU["NPU /dev/dri/by-path/platform-fde40000.npu-render"]
    MCU --> CSI["MIPI CSI-2（22-pin / 0.5 mm）"]
    CSI --> CAM["IMX219 摄像头（未启用）"]
    MCU --> I2S["I²S / 音频"]
    I2S --> HDMI["rockchip-hdmi 声卡（HAT 未接时唯一一张）"]
```

> **图 5**：Radxa ZERO 3W 主控到各总线与外设的连接关系，括号内为参考板实测的启用与接线状态。

> ⚠️ **NPU 设备节点以 by-path 为准，别记编号**。参考板实测
...
| **I²C（实测 I²C4 M0）** | 40-pin 的 3 / 5 | HAT 上 codec、ToF。**参考板 `fdtoverlays` 启的是 `rk3568-i2c4-m0` → `/dev/i2c-4`**（`i2cdetect -y 4`，实测总线在、无设备因 HAT 未接）。⚠️ OpenMicroDuck `main_controller.md` 原文写"I²C3 M0"，与参考板的 Radxa 设备树不符——以板子实测的 i2c-4 为准；原厂该控制器另一复用给了 USB-C PD，改到 M0 后失去 PD 协商，5 V 充电仍可用 |
| **MIPI CSI**   | 22-pin、0.5 mm 间距、4-lane 座 | **IMX219**（8MP，77° 裸模组；选购见 §3.0.2，overlay 见 §3.5） |
| **I²S**       | 40-pin                      | HAT 上 TLV320AIC3104 音频 codec（HAT 未接时 `cat /proc/asound/cards` 只有 `rockchip-hdmi` 一张卡，见 §3.6） |
...
> `scripts/setup-board.sh` 会 mask 这个 unit——**参考板已实测处理到位**：
...
> 一样要查。若再出现"突然全不认舵机"，先 `sudo fuser -v /dev/ttyS2` 看是不是 agetty
...
堵转 0.60 N·m @ 6 V、Dynamixel Protocol 2.0）。参考板改用 **飞特 Feetech HD-1910-C001**，
...
4. **换型号 = 重训策略**：官方 9 个 ONNX 是给 XL330 的 BAM M6 执行器模型训的，不能当
   ...
   训练/彩排命令统一带 `--motor hd1910`（§2.1 ③、§2.3），就是让 BAM 用 HD-1910 的参数
...
> 🔩 **轴承怎么买（核对 CAD 模型后给出的结论，数量有一处存疑，先看再下单）**
>
...
>   解析出来测量包围盒——`..._default` = 外径 15 × 厚 3（对应 6700K），`..._22x16x4` = 外径 22 × 厚 4
...
> 📷 **摄像头怎么下单（基于参考板 2026-09-18 实测给出的结论）**
>
...
> - **参考板三道硬件关的实测**：① `/boot/dtbo/` 里有
>   `radxa-zero3-rpi-camera-v2.dtbo`（IMX219）与 `-v1.3`（OV5647）两个 overlay，且**均为 `.disabled`**，
>   `fdtoverlays` 行里也没有 camera → 摄像头当前未启用、未接；
>   ② `/etc/iqfiles/` 里同时有 `imx219_RADXA-CAMERA-8M_default.json` 与
>   `imx219_rpi-camera-v2_default.json`，**没有 IMX519** 的 IQ 文件——所以商品页的
...
- **在哪里看**：`robotctl monitor` 的 IMU 投影重力项——三个值接近 (0,0,-1) 表示板子水平；
  明显偏移且跟随晃动，说明 IMU 活着且在正确工作；
- **跌倒判断**：`robotd` 用 IMU 重力投影决定"是否倒地"，所以**校准/安装方向错误会让机器人
...
> ⚠️ **注意：IMU 并不走独立的 I²C 总线。** 参考板实测（2026-09-18）：**`/dev/i2c-imu` 不存在**，
> `/etc/udev/rules.d/` 里也只有 `99-robot-i2c-pihat.rules`、没有 IMU 那条规则；而
> `/etc/robot/robotd.toml` 的 `[bus] port = "/dev/ttyS2"` 注释明确写着 15 个舵机与
> `imu_to_dxl` 板**共用这条串口**。因此"IMU 走独立的 `/dev/i2c-imu`（i2c-4、0x6A）"这一说法
> 在参考板上并未落地，排查时不要按它找设备。自查命令：

```bash
ls -l /dev/ttyS2 /dev/i2c-imu /dev/i2c-pihat   # i2c-imu 会报 No such file or directory
systemctl is-enabled serial-getty@ttyS2        # 实测 masked
```

> 💡 两个 IMU 别搞混：**总线上那个（id 200）是控制用的、必须开**；**ToF HAT 上那个是
> 可选的头部 IMU、默认关**。调错对象是新手最常见的"IMU 坏了"误报来源。

### 3.4 ToF【需硬件】：HAT 上的 8×8 深度矩阵
...
ToF 挂在 HAT 的 I²C 上，参考板对应的是 `/dev/i2c-4`（overlay `rk3568-i2c4-m0` 已启用），
**HAT 未接时该总线扫不到任何从机**（实测 `i2cdetect -y 4` 为空，全总线也扫不到 TLV320 的 0x18 与 ToF），
接上 HAT 后再按上面的顺序验证即可。
...
1. **CSI 摄像头必须配设备树 overlay**——不配的话插上也"没有任何反应"：不出现
   ...
   **参考板 Radxa OS（Debian 12，实测）**——overlay 是 `/boot/dtbo/` 里的文件，
   ...
   > 📎 **官方文档的 Armbian 做法（在 Radxa OS 上不适用，仅备查）**：Armbian 的 overlay 名靠
   ...
   > ⚠️ **未启用时设备节点就是这个样子**：参考板实测（2026-09-18）**`/dev/video0` 不存在**，
   > 只有 `/dev/video-dec0`、`/dev/video-enc0`；`/boot/dtbo/` 里 camera overlay 全是 `.disabled`，
   > `fdtoverlays` 里也没有 camera。看到这个形态即表示摄像头未启用、也未接。

2. `mediad` 必须启用——**没接摄像头时它找不到 `/dev/media*` 会退出、被 systemd 反复拉起、
   每十几秒烧满一核**（部署篇 §7.3 排障表有这条，参考板已把 mediad 停了：
   实测 `systemctl is-enabled mediad` = `disabled`）。接好
...
> ⚠️ **HAT 未接时没有独立声卡**：参考板实测（2026-09-18）`/proc/asound/cards` 只有
> `card 0 [rockchiphdmi]`，`aplay -l` 也只有 rockchip-hdmi。也就是说，在接上 HAT 之前
> 看不到 TLV320 codec 声卡，此时"没有音频输出设备"是正常现象，不代表驱动有问题。
...
| 突然全部舵机不认（之前好好的） | `sudo fuser -v /dev/ttyS2`                              | 出现`agetty` = serial-getty 复活占了总线；mask 掉（`sudo systemctl mask serial-getty@ttyS2`，参考板实测已 masked） |
...
| 负载莫名飙高                   | `systemctl show mediad -p NRestarts`                  | **没接摄像头时 mediad 崩溃循环**，先 `disable --now`，接好再 enable      |

## 4. 软件开发：改代码，推到板子上

> **本节目标**：读完这一节，可以做到**改一行代码、一分钟内在板子上看到它生效**，并且清楚推上去的版本是什么、出问题怎么退回来。整节围绕一个命令：`scripts/dev-push.sh` —— 它就是"从改一行到跑起来"的循环（全文见 `microduck/docs/robot/dev-push.md`）。

### 4.1 代码结构：一个 workspace，七个 daemon

`microduck` 是**单个 Rust workspace**（`microduck/README.md` L92-99），顶层结构：

```text
microduck/
├── Cargo.toml            # workspace；[workspace.metadata.rknpu] pin 了 NPU runtime 版本（§2.7）
├── .cargo/config.toml    # 定义 cargo board（交叉编译别名，§4.3 用）
├── robotctl/             # CLI：调试板子的唯一入口（§3 全用它）
...
**要改什么先想清楚：** 控制逻辑在 `robotd/src/`（`main.rs` 是主循环，`control.rs` 是策略驱动，`intents.rs` 是意图裁决）；命令契约在 `duck-ipc-proto`；CLI 在 `robotctl/src/`。**"加个命令给鸭子" = 协议加方法 + robotd 实现 + robotctl 接线**，三处都改。

```mermaid
flowchart LR
    WS["microduck：单个 Rust workspace"]
    WS --> RD["robotd：50 Hz 控制循环"]
    WS --> MD["mediad：摄像头/音频/WebRTC 远端"]
    WS --> UD["updaterd：签名、原子切换、健康门、回滚"]
    WS --> CD["configd：Wi-Fi、名字、配对"]
    WS --> PD["padd：手柄接收"]
    WS --> BT["btd：手机蓝牙连接"]
    WS --> TF["tofd：ToF 8×8 深度矩阵（只发不收）"]
    WS --> CL["robotctl / duckctl：CLI"]
    WS --> DD["duck-detect：检测器推理绑定"]
    WS --> IP["duck-ipc-proto：共享协议（JSON-RPC 2.0 NDJSON）"]
    WS --> SP["spaces：示例项目（Python）"]
```

> **图 6**：一个 workspace 中的各 crate 与板上 7 个常驻守护进程的对应关系。

### 4.2 一次性准备（只在第一台板子做一次）

dev-push 的门槛有三件（`microduck/docs/robot/dev-push.md` §Once）：

```bash
# ① 板子必须是 dev board（允许开发密钥）
#    若已按部署篇完成配置：/etc/robot/updater.toml 里 allow_dev_keys=true 且 team.dev.pub 已放好。
#    验证：sudo grep -c 'DEV BOARD' /var/lib/robot/provision.log   # 输出 1 为是

# ② 开发机放开发签名密钥（私钥，板子只留 .pub）
...
> 💡 **为什么必须签名？** 板子的 `updaterd` 对任何代码一视同仁：验证签名 → 校验哈希 → 检查兼容 → 过健康门。开发密钥签的包，客户板子会**拒收**（跟拒收任意 `--ref` 一样），反过来 dev 板也拒收任何没签名的东西。**没有"临时绕过"，这是设计不是麻烦。**
...
scripts/dev-push.sh --name duck-c51b        # 目标板子名，duckctl scan 可查
...
版本号 `0.5.1-dev.local.<时间戳>.<commit>`：**每次推送带时间戳，两次推送同一棵 dirty 树不会撞车**。板子上确认与回退：
...
# 开发机没有 zig 时用 Docker 构建（更慢，但零工具链要求）
...
sequenceDiagram
    ...
    participant Build as 交叉编译
    participant Sign as 打包签名
    participant Board as 板子
    participant UD as updaterd

    Dev->>Build: scripts/dev-push.sh
    Build->>Sign: cargo board --bins 交叉编译
    Sign->>Board: 拷贝到 ~/duck-sideload
    Board->>UD: robotctl update apply --from
    UD->>UD: 验证签名、校验哈希、检查兼容
    UD->>Board: 原子切换 current
    Board->>UD: 健康门检查
    alt 健康门通过
        UD-->>Dev: 新版本生效，各 daemon 均运行它
    else 健康门未通过
        UD->>UD: 自动回滚到上一个 release
        UD-->>Dev: 回滚完成
    end
```

> **图 7**：dev-push 的完整时序——交叉编译、打包签名、推送到板子、`updaterd` 原子切换，以及健康门通过与否的两条分支。

### 4.4 第一个真实改动：加一条"被摸"日志

找一处真实代码，把 debug 级别提到 info，推上去就能在 `journalctl` 里看到。位置：`robotd/src/main.rs` L2171-2182（抚摸检测事件处理）：
...
pet_detect::PettingEvent::Start => {
    ...
    if calm {
        ...
        // 👇 第一行改动：改成 info，这样不用开 debug 就能在日志里看到"被摸但忙"的事件
...
# journalctl 里出现 "petting detected (ignored: busy or down)" 即改动已在板上运行
```

> 📖 这条改动被选中，理由有三：**位置真实**、**改动最小**（一行）、**验证直观**（摸一下就有日志）。第一次 dev-push 用它建立"代码确实在板子上跑"的信心，比任何理论都强。

### 4.5 示例项目：spaces/ 里有什么

`spaces/` 是**独立的小项目**（不是 workspace 的一部分），演示"用现成接口拼功能"，全部是 Python、用 `uv` 跑。

**① policy-shop —— 策略商店**（`spaces/policy-shop/app.py` L13-16 定义了它依赖的四个调用）：

```text
policy.fetch    {repo, file}   → updaterd 下载并读 manifest      （板上用 robotctl policy add 等价）
...
界面上选一个策略 → 点 install → 点 run：上面四个调用按顺序各来一次。**这是"客户端怎么调鸭子"的最短教材**——四个调用就是 §2.5 那些 `robotctl policy …` 命令的底层方法名。

**② vision-demo —— 视觉流**：`DUCK_RECEIVER` 指向机器人摄像头帧的接收端，演示"不用板子也能看它看到什么"。跑法同样是 `uv run app.py`（`microduck/README.md` L81-98）。

**③ hello —— 最小演示**：最小可跑的"连上机器人 + 说句话"样板，看目录 README 即可。

> 💡 **spaces 和 daemon 的关系：** daemon 是板子上常驻的系统软件（Rust），spaces 是 PC/手机上的应用（Python）。**鸭子只提供 JSON-RPC 接口，怎么拼由应用决定。** 想加"遥控页面""自动投喂""语音对话"，都是在 spaces 这个层次做。
...
下表全部来自 `microduck/docs/robot/dev-push.md` 的 §When it does not work。
...
| `preflight check failed: SideloadDir ... not there` | 包放在 `/tmp`/`/var/tmp` 下 | `updaterd` 有 `PrivateTmp=yes`，看不到该拷贝；换 `~/duck-sideload` 等路径 |
...
> 把三块内容串成一个完整功能。演练分成"无需硬件"与"需硬件"两段：前者无需硬件即可完成，后者需接上硬件再收尾。每个阶段结束都有明确的"过关"标准——不过关不进下一阶段。
...
过关标准：health 全绿（mediad 无摄像头是预期状态，不算病）、版本 ≥ 0.5.0、`policies/current → seed-v5`、`detector/current → seed-duck-v1`。

### 阶段 1：在 PC 上走通"训练 → 导出 → 彩排"【无需硬件】（1 小时）
...
# 2) 导出（用官方 checkpoint；--checkpoint <iter> 填冒烟测试实际跑出的轮数）
...
过关标准：导出无警告、`rehearsal.json` 里 p50 延迟 < 20 ms、动作序列有限值。**不过关就先停在这一步**——调出问题的是配置，不是鸭子。
...
过关标准：`policy load` 返回成功（manifest 校验通过）、`policy list` 里 walk 槽显示该策略名。

> 💡 想省掉第 1 步也行：先 `scp` 把 ONNX 拷到板子，再 `sudo robotctl policy load walk /home/radxa/my_walk.onnx`（local 来源，同样持久生效）。**文件必须真的在板子上**，否则这个路径会被当成 `org/name` 仓库名而报 `is not an org/name repo`（见 §2.5 的判定规则）。发布只在需要"叫名字/绑手柄/分享/版本管理"时才有必要——见 §2.4 开头的分叉说明。

### 阶段 3：接上舵机、摄像头后的收尾【需硬件】

```bash
# 1) 把 mediad 开回来（§3.5：它因未接摄像头处于停用状态）
sudo systemctl enable --now mediad
...
#    观察 monitor 的事件行 + journalctl 里的 petting 日志（§4.4）
...
> 🎯 **这套演练的路线图意义：** 阶段 1 验证工具链是否打通、阶段 2 验证模型契约是否正确、阶段 3 验证真机行为。**每一步都能独立验收，卡在哪一步就去哪一节找答案**（阶段 1 → §2.1-2.3，阶段 2 → §2.4-2.5，阶段 3 → §3 + §4）。
...
**Q3：模型怎么才算"部署好了"？**
三条 `current` 链（§2.6）：`/opt/robot/{policies,detector,daemon}/current` 指向目标版本，
且 `robotctl policy list` / `robotctl duck-detector check` 显示已加载。**模型不重启 daemon，代码要重启。**

**Q4：板子没接摄像头，为什么 CPU 负载会飙高？**
mediad 崩溃循环（部署篇 §7.3 排障表 + §3.7）：找不到 `/dev/media*` 退出，systemd 反复拉起。
...
SSH 上板：`sudo robotctl update rollback daemon`。再不行就上板刷机（部署篇兜底）。
...
| 想读什么                                    | 文件                                                                        |
| ------------------------------------------- | --------------------------------------------------------------------------- |
| 系统总览、7 daemon 职责、通信方式           | `microduck/docs/design/architecture.md`                                     |
| robotd 设计：50 Hz 循环、舵机总线、IMU 读取 | `microduck/docs/design/robotd-design.md`                                    |
| 更新引擎：签名、原子切换、健康门、回滚      | `microduck/docs/design/update-pipeline.md`（如缺看 updaterd 源码）          |

**模型**

| 想读什么                             | 文件                                                            |
| ------------------------------------ | --------------------------------------------------------------- |
| NPU 上板全记录（性能、转换、阈值）   | `microduck/docs/project/npu-bringup.md`                         |
| 策略清单格式（manifest 契约）        | `microduck/docs/policy-manifest.md`                             |
| 循环策略（LSTM、model_api: 2、彩排） | `microduck/docs/recurrent-policies.md`                          |
| 训练配置 + 命令（HD1910）            | `microduck_rl/docs/training_config_velocity_hd1910.md`          |
| 训练仓命令与铁律                     | `microduck_rl/AGENTS.md`                                        |

**硬件**

| 想读什么                              | 文件                                               |
| ------------------------------------- | -------------------------------------------------- |
| 板子速查（health/monitor 等全部命令） | `microduck/docs/robot/cheatsheet.md`               |
| ToF 上板与 IMU 配置决策               | `microduck/docs/project/tof-on-demand.md`          |
| 摄像头/媒体上板（overlay 坑）         | `microduck/docs/project/media-bringup.md`          |

**开发**

| 想读什么                             | 文件                                                |
| ------------------------------------ | --------------------------------------------------- |
| dev-push 全流程（本教程 §4 的原文） | `microduck/docs/robot/dev-push.md`                  |
| 开发板信任机制（dev key）            | `microduck/docs/robot/install-dev.md`               |
| 找板子/SSH/SCP（蓝牙兜底）           | `microduck/docs/robot/duckctl.md`                   |
| 开发板速查（重启陷阱等）             | `microduck/docs/robot/cheatsheet-dev.md`            |
...
**OpenMicroDuck 资料**（`OpenMicroDuck/docs/`，本教程 §3.0.1/§3.0.2 的参考来源）
...
| Hugging Face Hub（策略与检测器模型分发）      | [https://huggingface.co](https://huggingface.co)                                                         |
| crates.io 国内镜像（板端编译用）              | [https://rsproxy.cn](https://rsproxy.cn)                                                                 |
| ONNX Runtime（板端推理运行时）                | [https://github.com/microsoft/onnxruntime](https://github.com/microsoft/onnxruntime)                     |
| Rockchip RKNN 工具链                          | [https://github.com/airockchip/rknn-toolkit2](https://github.com/airockchip/rknn-toolkit2)               |

---

## 待核实事项

以下是整理过程中发现的、**源文档前后不一致或可能已过时**的点。为避免擅自改动数字，这里只如实列出，未在正文中修改；实际执行时请以命令输出为准。本节按核实状态分两组：第一组已在参考板上用只读命令核实并回填事实，第二组需要额外硬件或离线资料，暂无法在板上验证。

**已核实（2026-09-18 板端实测）** —— 下列条目已用只读命令在参考板上核实，逐条给出结论、关键回显与用到的命令：

1. **NPU 设备节点编号**：稳定标识应使用 by-path 路径，不要写死编号。用到的命令 `ls -l /dev/dri/by-path/`；关键回显 `platform-fde40000.npu-render -> renderD129`（同目录下 `platform-display-subsystem-render -> renderD128`、`platform-fde60000.gpu-render -> renderD130`）。→ NPU 对应 renderD129，但编号会随枚举变化，脚本中请用 by-path 路径而非固定编号。
2. **I²C 总线与 IMU 拓扑**：用到的命令 `ls /dev/i2c-*`；关键回显只有 0 / 3 / 4 / 6，**没有 i2c-5**。i2c-3 上是 Type-C 控制器 `3-0022 fusb302 [typec_fusb302]`；i2c-0 上是 PMIC `0-0020 rk817 [rk808]` 与 `0-0040 syr827 [fan53555-regulator]`；i2c-6 是 `DesignWare HDMI`（DDC）；i2c-4 无从机。已启用 overlay 为 `rk3568-i2c4-m0.dtbo`（见 `/boot/extlinux/extlinux.conf` 的 `fdtoverlays` 行），而 `rk3568-i2c3-m0.dtbo` 在 `/boot/dtbo/` 里是 `.disabled`。**参考板上不存在 `/dev/i2c-imu`**；`/etc/udev/rules.d/` 只有 `99-robot-i2c-pihat.rules`（生成 `/dev/i2c-pihat -> i2c-3`），而没有 IMU 对应的那条规则。`/etc/robot/robotd.toml` 使用 `port = "/dev/ttyS2"`（`/dev/ttyS2` 存在：`crw-rw---- root dialout 4,66`），其注释说明 15 个舵机与 imu_to_dxl 板共用该串口；`serial-getty@ttyS2` 实测为 `masked`。→ 因此"IMU 走独立 `/dev/i2c-imu`（i2c4、0x6A）"的表述与参考板不符，以实测为准。
3. **摄像头与摄像头 overlay**：用到的命令 `ls /dev/video*`；关键回显 `/dev/video0` 不存在，只有 `/dev/video-dec0`、`/dev/video-enc0`。`/boot/dtbo/` 中 camera overlay（rpi-camera-v2 / radxa-camera-8m）全为 `.disabled`，且 `extlinux` 的 `fdtoverlays` 未含 camera → 摄像头 overlay 未启用（也未接）。`ls /etc/iqfiles/` 只有 imx219 系列（`imx219_RADXA-CAMERA-8M_default.json`、`imx219_rpi-camera-v2_default.json`），**无 IMX519**。
4. **音频**：未接 HAT 时 `cat /proc/asound/cards` 关键回显只有 `card 0 [rockchiphdmi]`，`aplay -l` 同样只有 rockchip-hdmi，**没有 TLV320 codec 声卡**。
5. **eMMC 与 mmcblk 编号**：用到的命令 `lsblk`；关键回显只有 `mmcblk1`（14.6 G microSD，`mmcblk1p1` 16M → `/config`、`mmcblk1p2` 300M → `/boot/efi`、`mmcblk1p3` 14.3G → `/`），**没有 mmcblk0 / mmcblk2**。→ 无 eMMC 的板子上 microSD 即 `mmcblk1`；"有 eMMC 的板子 SD 卡走 mmcblk0"这条因手上没有 eMMC 板而**未验证**。

**仍待核实（需要硬件或离线资料）** —— 下列条目暂无法用板端只读命令验证，原因逐条注明：

1. **轴承数量冲突**：`OpenMicroDuck/docs/bom.md` 写 ET2216 ×1，而 `robot_openmicroduck_groundcontact.xml` 里 16×22×4 出现 11 次。属 CAD/BOM 问题，板子上验不了，下单前需看装配图或向作者确认。
2. **IMX219 像素数**：`OpenMicroDuck/docs/bom.md` 写"500 万"，而 IMX219 官方规格是 800 万（3280×2464）。需实物确认后缀型号。
3. **训练仓文档路径**：训练用的 `microduck_rl` 仓库里 `docs/training_config_velocity_hd1910.md` 与 `AGENTS.md` 的实际路径可能随上游变动，引用前请先在仓库内确认。
4. **上板用 `update-pipeline` 设计文档路径**：文档中引用的 `microduck/docs/design/update-pipeline.md` 在部分版本中缺失，若不存在请直接看 `updaterd` 源码。
5. **性能实测数字的平台标注**：相关数字来自 PC 侧（CPU 彩排）还是板端 NPU，需在正式发布前逐条标注清楚平台，避免读者误读。
6. **麦克风/扬声器料号**：官方链路是"模拟麦克风 → TLV320AIC3104 codec → PAM8406D 功放 → 无源喇叭"，需确认所选麦克风是**模拟输出**（MEMS 或驻极体电容麦），扬声器是**无源小喇叭**（8 Ω / 1–2 W）；不能用 I²S/PDM 数字麦（如 ICS-43434、SPH0645、SPM1423）或带 3.5 mm/USB 的有源音箱。
7. **章节号交叉引用**：部署篇与开发篇之间的章节号交叉引用（如"部署篇 §7.3"）在最终发布前需按两篇的实际编号再核一遍。
...
| v1.01 | 2026-09-18 | ① 正文按对外发布稿体裁重排并补齐 §1–§7 与文末三节；② 新增 **8 张 Mermaid 结构图**（三条链路总览、7 个守护进程、模型链路、彩排双路径、NPU 推理链路、硬件拓扑、workspace 与 daemon 对应、dev-push 时序），并把 §0.1 的 ASCII 路线图改为 Mermaid；③ 按板端只读实测核实并回填硬件事实（I²C 总线与 IMU 拓扑、NPU 设备节点、摄像头与音频枚举、eMMC/mmcblk 编号），「待核实事项」按"已核实 / 仍待核实"分组 |
